...
For successfully repaired, parseable programs, \(\mathrm{TED}_{B\rightarrow F}\) measures AST distance from current to fixed code and \(\mathrm{TED}_{F\rightarrow O}\) from fixed code to the recorded accepted oracle, using the standard ordered-tree edit formulation~\cite{zhang1989simple}. RQ2 uses \(\mathrm{TED}_{F\rightarrow T}\), where \(T\) is the identical adapter-specific recorded target shared by Full and Current configurations.
...
We presented \tool, an execution-guided APR framework trained from authentic student submission trajectories. Nested execution-evidence orders expose monotonic testcase progress and verdict improvement, while terminal closure supplies accepted targets. Their finite-chain, inclusion, selection-safety, and minimum-escalation properties are proved and mechanically verified. Independent-policy composition raises Seen repair rate from 50.1\% for the strongest individual adapter to 59.5\%, while early stopping averages 2.04 candidates. The system outperforms trajectory-prompted zero-shot repair on Seen and problem-held-out Unseen data under problem-clustered inference and produces smaller successful patches, although same-problem retrieval achieves higher coverage. Three seeds yield Seen and Unseen RR standard deviations of 1.0 and 1.9 points.

Target-matched adapters retain the benefit without serialized histories: trajectories provide executable reachability relations distilled into parameters, while the certified portfolio converts complementary relations into minimum-breadth repairs.

\section{Data Availability}
\label{sec:data-availability}

An anonymized artifact will provide code, prompts, splits, outputs, and a SHA-256 manifest for 143 records (2.29 GB) at revision \texttt{aa908288a397}. Project CodeNet will be reacquired from its source; deterministic regeneration and checksum instructions will be included.

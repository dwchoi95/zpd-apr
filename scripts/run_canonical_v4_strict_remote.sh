...
CHECKPOINT="${WORK_ROOT}/checkpoints/split-90-10/canonical-v4/strict"
LOG="${RUN_ROOT}/logs/train-strict.log"
GENERATIONS="${RUN_ROOT}/eval/strict-seen-test.generations.jsonl"
EVALUATION="${RUN_ROOT}/eval/strict-seen-test.evaluation.jsonl"
ANSWER_SERVICE=zpd-canonical-v4-answer-eval-r1.service
...
"${PYTHON}" run.py train-qlora \
  ...
  --learning-rate 2e-4 \
  --edit-token-weight 1 \
  --validation-dataset "${DATASET_ROOT}/valid-strict.jsonl" \
  --eval-steps 100 \
...
while systemctl --user is-active --quiet "${ANSWER_SERVICE}"; do
  sleep 60
done
